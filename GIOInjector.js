...
function reactNativeNavigationScreenString(){
    ...
    if (event.id == "willAppear") {
		require('react-native').NativeModules.RNPyyRnLib.onPagePrepare(growingPageName);
    } else if (event.id == "didAppear") {
        require('react-native').NativeModules.RNPyyRnLib.onPageShow(growingPageName);
    }`;
	return script;
}
...
`;
	...
	require('react-native').NativeModules.RNPyyRnLib.onPageShow(pageName);}`;
	return script;
}
	...
	`;
		...
		if (require('react-native').Platform.OS === 'android') {
			...
			if (!${actionName} || iosOnPageShow) {
				...
				require('react-native').NativeModules.RNPyyRnLib.onPagePrepare(pageName);
			}
		}`;
		return script;
}
...
function createViewTransformer(content){
	...
	var functionBody =
		...
		var params = ${propsName}.SAParams;
         ...
         require('react-native').NativeModules.RNPyyRnLib.prepareView(${tagName
         }, clickable, params);
                 `;
